...
class FSQ(nn.Module):
    """Minimal FSQ (https://arxiv.org/abs/2309.15505) implementation. Expects channel dimension to be last.
    ...
    def unbound(self, z: Tensor, eps: float = 1e-3, epsi: float = 1e-3) -> Tensor:
        """Unbinds `z`, an array of shape (..., d)."""
        half_l = (self._levels - 1) * (1 + eps) / 2
        offset = torch.where(self._levels % 2 == 0, 0.5, 0.0)
        shift = (offset / half_l).atanh()
        return ((z + offset) / half_l).clamp(min=-1 + epsi, max=1 - epsi).atanh() - shift

    def quantize(self, z: Tensor, quantize: bool = True) -> Tensor:
    ...
    def quantize_j(self, z: Tensor, quantize: bool = True) -> Tensor:
        """Quantizes z, returns quantized zhat, same shape as z."""
        bounded = self.bound(z)
        drop_quant_p = self.drop_quant_p if self.training else 0.0
        if not quantize:
            drop_quant_p = 1.0
        quantized = round_ste_quant_dropout(bounded, drop_quant_p)
        return z + (self.unbound(quantized) - z).detach()
    ...
    def embedding(self, indices: Tensor) -> Tensor:
        """Overload for the inverse of `codes_to_indices`."""
        return self.indices_to_embedding(indices).to(self.dtype)
    ...
    def corrupt_quant(self, quant: Tensor) -> Tensor:
        ...
        random_indices = torch.randint(low=0, high=self.codebook_size, size=quant_shape, device=quant_device)
    ...
    def forward_z(self, z: torch.Tensor, quantize: bool = True) -> Tensor:
        assert z.shape[-1] == self.dim, f"expected dimension of {self.dim} but found dimension of {z.shape[-1]}"

        quant = self.quantize(z.float(), quantize=quantize)
        if self.training and self.corrupt_tokens_p > 0.0 and random.random() < self.apply_corrupt_tokens_p:
            # Optionally corrupt a random percentage of tokens during training
            quant = self.corrupt_quant(quant)

        return quant

    def forward(self, z: Tensor, quantize: bool = True) -> tuple[Tensor, Tensor]:
        quant = self.forward_z(z, quantize=quantize).to(self.dtype)

        # quantized tokens, vq_loss, aux_output
        return quant, 0, None
